...
export default function Gallery() {
  ...
  useEffect(() => {
    // Fetch Instagram media (server-side proxy)
    const fetchIG = async () => {
      try {
        const res = await fetch("/api/instagram?limit=100", { cache: "no-store" });
        const json = await res.json();
        if (json && Array.isArray(json.data)) {
          // Filter unsupported or missing URLs
  ...
  const expandedItem = document.createElement("div");
    ...
    // Actions: Instagram link if available
