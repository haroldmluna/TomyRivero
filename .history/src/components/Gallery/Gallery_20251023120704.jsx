...
export default function Gallery() {
  ...
  useEffect(() => {
    // Fetch Instagram media and Threads embeds (server-side proxy)
    const fetchSocial = async () => {
      try {
        const [igRes, threadsRes, igOembedRes] = await Promise.all([
          fetch("/api/instagram?limit=100", { cache: "no-store" }),
          fetch("/api/threads", { cache: "no-store" }),
          fetch("/api/instagram/oembed", { cache: "no-store" }),
        ]);

        const igJson = await igRes.json().catch(() => ({}));
        const thJson = await threadsRes.json().catch(() => ({}));
        const igOembedJson = await igOembedRes.json().catch(() => ({}));

        const fromIG = Array.isArray(igJson?.data)
          ? igJson.data
        ...
        const fromThreads = Array.isArray(thJson?.data)
          ? thJson.data
              .filter((m) => m.media_url || m.thumbnail_url)
              .map((m) => ({
                id: m.id,
        ...
        const fromIGOembed = Array.isArray(igOembedJson?.data)
          ? igOembedJson.data
              .filter((m) => m.media_url || m.thumbnail_url)
              .map((m) => ({
                id: m.id,
        ...
        const merged = [...fromIG, ...fromThreads, ...fromIGOembed];
        setIgMedia(merged);
        setIgReady(merged.length > 0);
      } catch (e) {
        console.warn("Social fetch failed", e);
      }
    };
  ...
  const getIGByIndex = (index) => {
    if (!igReady || igMedia.length === 0) return null;
    const i = Math.abs(index) % igMedia.length;
    return igMedia[i] || null;
  };
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (stateRef.current.expandedItem && stateRef.current.expandedItem.parentNode) {
  ...
  const itemCount = Math.max(igMedia.length || 0, 20);
  const itemGap = 150;
  const columns = 4;
  const itemWidth = 120;
  ...
  const updateVisibleItems = () => {
    ...
    for (let row = startRow; row <= endRow; row++) {
      for (let col = startCol; col <= endCol; col++) {
        ...
        const linearIndex = Math.abs(row * columns + col);
        const igObj = getIGByIndex(linearIndex);
        ...
        if (igObj && igObj.url) {
          ...
          if (igObj.caption) item.dataset.caption = igObj.caption;
          if (igObj.permalink) item.dataset.permalink = igObj.permalink;
        } else {
          const itemNum = (linearIndex % 20) + 1;
          img.src = `/archive/archive-${itemNum}.jpg`;
        }
        img.alt = `Image ${linearIndex + 1}`;
  ...
  const expandItem = (item) => {
    ...
    try { const { track } = require("@/lib/analytics"); track && track("gallery_expand", { source: item?.dataset?.permalink ? "instagram" : "local" }); } catch {}
  ...
  const igCaption = state.activeItem?.dataset?.caption;
  const titleText = igCaption && igCaption.trim().length > 0 ? igCaption : items[titleIndex];
  ...
  const expandedItem = document.createElement("div");
    ...
    // Actions: link if available (Instagram or Threads)
    const actions = document.createElement("div");
    actions.className = "expanded-actions";
    const permalink = state.activeItem?.dataset?.permalink;
    if (permalink) {
      const link = document.createElement("a");
      link.href = permalink;
      link.target = "_blank";
      link.rel = "noreferrer";
      link.className = "btn-instagram";
      const isThreads = /threads\.net/.test(permalink);
      const label = isThreads ? "View on Threads" : "View on Instagram";
      link.innerHTML = `
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          <path d="M7 2h10a5 5 0 0 1 5 5v10a5 5 0 0 1-5 5H7a5 5 0 0 1-5-5V7a5 5 0 0 1 5-5zm5 5a5 5 0 1 0 0 10 5 5 0 0 0 0-10zm0 2.2a2.8 2.8 0 1 1 0 5.6 2.8 2.8 0 0 1 0-5.6zM18.4 6.2a1.2 1.2 0 1 0 0-2.4 1.2 1.2 0 0 0 0 2.4z" fill="#111"/>
        </svg>
        <span>${label}</span>
      `;
      link.addEventListener("click", (ev) => { ev.stopPropagation(); try { const { track } = require("@/lib/analytics"); track && track("gallery_instagram_click"); } catch {} });
      actions.appendChild(link);
    }
    expandedItem.appendChild(actions);

    // Caption with read more
    const captionText = state.activeItem?.dataset?.caption || "";
    if (captionText) {
      const captionWrap = document.createElement("div");
      captionWrap.className = "expanded-caption";
      const captionP = document.createElement("p");
      const maxLen = 140;
      const full = captionText;
      const short = full.length > maxLen ? full.slice(0, maxLen).trimEnd() + "…" : full;
      let expanded = false;
      captionP.textContent = short;
      ...
      if (full.length > maxLen) {
        ...
        moreBtn.addEventListener("click", (ev) => {
          ev.stopPropagation();
          expanded = !expanded;
          captionP.textContent = expanded ? full : short;
          moreBtn.textContent = expanded ? "Less" : "More";
        });
        captionWrap.appendChild(moreBtn);
      }

      captionWrap.appendChild(captionP);
      expandedItem.appendChild(captionWrap);
    }
