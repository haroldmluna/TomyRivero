...
export default function ParallaxBanner() {
  ...
  // Ajusta perspectiva en el contenedor
  gsap.set(root, { perspective: 1200 });

  layers.forEach((layerEl) => {
    const depth = parseFloat(layerEl.getAttribute("data-depth") || "1");
    const scaleStart = 1 + depth * 0.2; // zoom inicial (más profundo, más cerca)
    const scaleEnd = 1; // regresa a escala normal

    gsap.fromTo(
      layerEl,
      { scale: scaleStart, transformOrigin: "center center", z: depth * -300 },
      {
        scale: scaleEnd,
        z: 0,
        ease: "none",
...
const layers = [
  { src: Background, alt: "Background", depth: 0.4, z: 0, offsetY: 0 },
  { src: Layer1, alt: "Layer 1", depth: 0.6, z: 5, offsetY: 0 },
  { src: Layer2, alt: "Layer 2", depth: 0.8, z: 4, offsetY: 10 },
  { src: Layer3, alt: "Layer 3", depth: 1.0, z: 3, offsetY: 20 },
  { src: Layer4, alt: "Layer 4", depth: 1.2, z: 2, offsetY: 30 },
  { src: Layer5, alt: "Layer 5", depth: 1.4, z: 1, offsetY: 15 },
];
  ...
  style={{
    zIndex: l.z,
    transform: `translateY(${l.offsetY}px)`,
  }}
  data-depth={l.depth}
  key={l.alt}
>
